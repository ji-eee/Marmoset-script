...
- `install.ps1`에 한글/비ASCII 문자 넣기 → Windows PowerShell 5.1은 BOM 없는 UTF-8을
  ANSI로 읽어 글자가 깨짐. 설치 스크립트 메시지는 ASCII(영어)로 유지하고 한글 안내는
  README에. `.bat`/`.ps1`은 `.gitattributes`로 CRLF 체크아웃 고정
