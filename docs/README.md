...
| 텍스처 사이즈 | 프리셋(512/1024/2048/4096), 기본값 1024 |
| UV를 적용할 타겟 메쉬 지정 | Target Mesh: 기본은 캡처 메쉬 자신, 또는 다른 메쉬(미드→로우 리프로젝션) 선택 |
| 캡처 방식 선택 | Capture Mode: Front+Back(기본, 턴테이블) 또는 Current view(현재 화면 단일 투영) |
...
권장: 저장소 루트의 설치 스크립트 사용(자세한 사용법은 최상위 [README](../README.md) 참고)

```powershell
irm https://raw.githubusercontent.com/ji-eee/Marmoset-script/main/install.ps1 | iex
```

또는 clone 후 `install.bat` 더블클릭.
git 확보(winget 설치/ZIP 폴백) → 플러그인 폴더 자동 탐색(실패 시 경로 입력) → 복사까지 한 번에 처리.

수동 설치 시 `CaptureFrontBackBake.py` 와 `projbake/` 폴더를 같은 위치에 둔 채로 복사
...
3. (선택) Target Mesh — 기본은 캡처 메쉬 자신, 미드→로우면 로우폴리를 숨기고 타겟으로 지정
4. (선택) Capture Mode — Front+Back(기본) 또는 Current view(현재 화면 단일 투영)
5. Texture Size(프리셋 512~4096, 기본 1024), Side Mask Angle(기본 75도),
   Edge Blur(px, 기본 3, 0이면 끔) 확인
6. Capture Front/Back And Bake 클릭
7. 출력 폴더에 메테리얼마다 2장 생성(중간 캡처 파일은 자동 삭제됨)
